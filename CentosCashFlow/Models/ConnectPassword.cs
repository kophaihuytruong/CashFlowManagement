...
namespace CentosCashFlow.Models
{
    public class ConnectPassword
    {
        ...
        public static bool verify(string password, string hashPassword, string algorithm = "bcrypt")
        {
            ...
            if (algorithm == "md5") return ConnectPassword.Create_MD5(password) == hashPassword;
            if (algorithm == "sha1") return ConnectPassword.Create_SHA1(password) == hashPassword;
            if (algorithm == "sha256") return ConnectPassword.Create_SHA256(password) == hashPassword;
            return false;
        }
        public static string Create_MD5(string text)
        {
            return ConnectPassword.Hash(text, new MD5CryptoServiceProvider());
